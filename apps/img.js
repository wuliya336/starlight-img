...
export class img extends plugin {
  ...
  async imglist(e) {
    ...
    await Common.render('img/list', {
      imglist: imageList
    }, { e, scale: 1.4 });
  }

  async imgnum(e) {
    const match = e.msg.match(/(#)?(星点图片|starlight-img)(随机)?(\S+)(图片)?(数量)$/i);
    ...
    let imageList = await getImageList();
    let selectedImage = imageList.find(item => item.name === name || item.alias === name);
